...
// 삭제버튼 클릭 시 해당 TODO 삭제제
const delBtnElem = (todoId) => {
...
// 두개 가 같으면 모두 완료된 상태 -> 버튼 업데이트트
const checkIsAllCompleted = () => {
    if (getAllTodos().length === getCompletedToods().length) {
...
// 버튼 클릭 시 토글 처리 함수
const oonClickCompleteAll = () => {
    if (!getAllTodos().length) return; // 할 일이 하나도 없으면 종료
    if (isAllCompleted) incompleteAll(); // 현재 isAllCompleted 값에 따라 전체완료 & 전체 미완료 처리리
    else completeAll(); 
...
// 남은 할 일 수 세기
const setLeftItems = () => {
    const leftToods =getActiveTodos(); // 완료 되지 않은 할 일들만 가지고 옴
    // 요소에 해당값을 텍스트로 넣음
...
const onDbclickTodo = (e, todoId) => {
    ...
    const inputText = e.target.innerText;  // inputText: 현재 보이는 텍스트를 읽어옴 -> <div>안의 텍스트트
    const inputElem = todoElem.createElement('input');  // inputElem: 새로운 <input> 테그를 만듦
    ...
    inputElem.addEventListener('keypress', (e) => {
        if (e.key === 'Enter') { // 사용자가 엔터를 누르면
            updataTodo(e.target.value, todoId); //  updataTodo를 호출해 내용 변경
            document.body.removeEventListener('click', onClickBody); // 클릭이벤트 리스너 헤제
        }
    });
...
// 수정 내용 반영
const updataTodo = (text, todoId) => {
    const currentTodos = getAllTodos(); 
    // 변경된 배열을 다시 설정 (id가 일치해야함함)
...
const paintTodos = () => {
    ...
    switch (currentShowType) {
        ...
        case 'active':
            ...
            break;
        case 'completed':
            const completedTodos = getCompletedToods();
